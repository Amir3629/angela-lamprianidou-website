'use client';

import MediaGallerySeparated from "../../../components/MediaGallerySeparated";
import Header from "../../../components/Header";

export default function BackToEmotionPage() {
  const backToEmotionMedia = [
    {
      src: "/backtoemotion-1.jpg",
      alt: "Back to Emotion",
      type: "image" as const,
      title: "Back to Emotion"
    },
    {
      src: "/backtoemotion-2.jpg", 
      alt: "Personal Interactive Meeting",
      type: "image" as const,
      title: "Personal Interactive Meeting"
    },
    {
      src: "/backtoemotion-3.jpg",
      alt: "One-Person Audience",
      type: "image" as const, 
      title: "One-Person Audience"
    },
    {
      src: "/backtoemotion-4.jpg",
      alt: "Private Flat Performance",
      type: "image" as const,
      title: "Private Flat Performance"
    },
    {
      src: "https://www.youtube.com/watch?v=a3GLyclqvOM",
      alt: "Back to Emotion Video",
      type: "video" as const,
      title: "Back to Emotion Video"
    }
  ];

  const description = `In back to emotion, A. Lamprianidou is proposing a performance which 
is based on a personal interactive meeting with a one-person audience. 
Disassociating herself from the distance we're given in a common stage. 
Because of this the performance will 
take place not in a theater or on a 
stage, it will happen in her own flat. 
This is a big challenge not only 
for the audience but also for 
the performer herself.`;

  const additionalInfo = `What will happen ? 
Anything or everything ? 
Cava or champagne ? 
Marc Almond or Andrew Williams ? 
Dance me dance you dance we ? 
Video  https://www.youtube.com/watch?v=a3GLyclqvOM
More information on 
www.lamprianidou.com 
Reserve youre personal entry 
call +4917663459337 

and we clear up date and hour, 
like it should be,no¿ 
No press allowed, 
no mobile phones.`;

  

  return (
    <div className="min-h-screen bg-white text-black website-font white-bg font-variation-1 media-screen-small angela-typography">
      <Header />
      <MediaGallerySeparated
        mediaItems={backToEmotionMedia}
        title="Back to Emotion"
        description={description}
        additionalInfo={additionalInfo}
        photographer="© Photos by Angela Lamprianidou"
        imageSize="xlarge"
      />
    </div>
  );
}